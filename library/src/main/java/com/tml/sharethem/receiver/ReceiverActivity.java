...
public class ReceiverActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LR_topAd = (AdView) findViewById(R.id.LR_topAd);
        LR_bottomAd = (AdView) findViewById(R.id.LR_bottomAd);
        MobileAds.initialize(this, getString(R.string.ApAdId));
        AdRequest adRequest=new AdRequest.Builder().build();
        LR_topAd.loadAd(adRequest);
        LR_bottomAd.loadAd(adRequest);
    ...
    private void buildAlertMessageNoGps() {
        ...
        builder.setMessage("Your GPS seems to be disabled, Please enabled it to proceed with sharing")
                .setCancelable(false)
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION) && !WifiUtils.isWifiConnectedToSTAccessPoint(getApplicationContext())) {
                ...
                if (ConnectivityManager.TYPE_WIFI == netInfo.getType()) {
                    ...
                    //change adding new lines (najam)
                    final String ip1 = WifiUtils.getAccessPointIpAddress(getApplicationContext());
                    addSenderFilesListingFragment(ip1, info.getSSID());
                    if (null == preferences)
